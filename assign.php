...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST["assign_seva_coordinator"])) {
        ...
        if (!$assignment_exists) {
            ...
            $insertStudentQuery = "INSERT INTO seva_assignments (`Student ID`, `Seva Id`) VALUES (?, ?)";
            ...
            if (mysqli_stmt_execute($stmtSelect)) {
                ...
                foreach ($unique_student_ids as $student_id) {
                    mysqli_stmt_bind_param($stmtStudent, "ii", $student_id, $seva_id);
    ...
    // If the form was not submitted via POST, redirect to the previous page
    $redirect_url = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : 'login.php';
    header('Location: ' . $redirect_url);
    exit();
}
