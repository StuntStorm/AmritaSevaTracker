    ...
    if (isset($_POST["assign_seva_coordinator"])) {
        ...
        if (mysqli_num_rows($result) == 0) {
            ...
            if (mysqli_stmt_execute($stmtSelect)) {
                ...
                // Insert data into seva_assignments table
                $insertQuery = "INSERT INTO seva_assignments (Seva Id, Faculty ID, StartTime, EndTime) VALUES (?, ?, ?, ?)";
                ...
                mysqli_stmt_bind_param($stmtInsert, "iiss", $seva_id, $faculty_id, $start_time, $end_time);

                if (mysqli_stmt_execute($stmtInsert)) {
                    // Insert student assignments with their corresponding SIDs
                    foreach ($students as $student) {
                        $parts = explode('|', $student);
                        $student_name = $parts[0];
                        $sid = $parts[1];

                        $insertStudentQuery = "INSERT INTO seva_assignments (Student ID) VALUES (?)";
                        ...
                        mysqli_stmt_bind_param($stmtStudent, "i", $sid);
